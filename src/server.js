...
const login = ctx => {
  ...
  const payload = { iss: userName, exp: expires };
  const Token = jwt.encode(payload, SECRET);  // 加密
  ctx.response.body = { data: Token, msg: '登陆成功',status: 200 };
}

const getUserName = ctx => {
  const token = ctx.get('authorization').split(" ")[1]; // 获取请求头，不区分大小写
  const payload = jwt.decode(token, SECRET);  // 解密
  if (Date.now() > payload.exp) {   // 每次请求只判断Token是否过期，不重新去更新Token过期时间(更新不更新Token的过期时间主要看实际的应用场景)
    ctx.response.body = { errorMsg: 'Token已过期，请重新登录' };
  } else {
    ctx.response.body = {
      data: { username: payload.iss },
      msg: '获取用户名成功',
      errorMsg: ''
    };
  }
